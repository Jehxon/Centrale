...
float length(cv::Mat a)
{
	//Computes the length of a 3*1 vector
	return(sqrt(a.dot(a)));
}
...
void drawHouse(float size, bool occupied)
{
	...
	{
		glColor3f(0.2,0.9,0.5); //green color to match the caracters
	}
...
{
	//Draws a little geometric human
...
void ArUco::drawScene() {
   ...
   for (unsigned int m=0;m<m_Markers.size();m++)
   {
	  ...
	  if(m != last) //The house will be on the last marker (once we know if it's occupied or not)
	  {
		  //We compute the distance between the 2 markers
		  Mat vecDist = m_Markers[m].Tvec - m_Markers[last].Tvec;
		  
		  if(length(vecDist) < m_MarkerSize * 1.5) //If the distance is lesser than the length of the marker, the house is occupied and we do not draw the caracter
		  {
			  occupied = true;
		  }
		  else
